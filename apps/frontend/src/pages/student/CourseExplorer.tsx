...
export default function Courses() {
  ...
  const filtered = useMemo(() => {
    if (categoryIndex === 0) return courses;
    const year = categoryIndex;
    return courses.filter((c) => c.acadamicYear === year);
  }, [courses, categoryIndex]);
  ...
  if (error && courses.length === 0) {
    return (
      <div className="p-8 text-error text-sm">{error}</div>
    );
  }
  ...
  return (
    <div className="p-8 space-y-8">
      <section className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-medium tracking-tight transition-all ${
              i === categoryIndex
                ...
                : 'bg-surface-high text-on-surface-variant hover:text-white border border-outline-variant/10'
            }`}
          >
            {cat}
          </button>
        ...
        <div className="ml-auto flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <button type="button" className="flex items-center gap-1 text-xs font-medium text-on-surface hover:text-primary transition-colors">
              Latest
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {featured ? (
          <div
            className="lg:col-span-8 group relative overflow-hidden rounded-sm bg-surface-low p-1 border border-outline-variant/5 cursor-pointer"
            ...
            <div className="relative h-100 w-full overflow-hidden rounded-sm">
              ...
              <div className="absolute bottom-0 left-0 p-8 w-full">
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-2 py-0.5 rounded-sm bg-secondary/20 text-secondary text-[10px] font-mono font-bold tracking-tighter uppercase border border-secondary/30">
                    Course Spotlight
                  </span>
                  <span className="text-on-surface-variant font-mono text-[10px]">
                    Year {featured.acadamicYear} · {featured.code}
                  </span>
                </div>
                ...
                <p className="text-on-surface-variant max-w-xl text-sm mb-8 leading-relaxed">
                  {featured.instructorName || featured.instructorId} · {MVP_DEPARTMENT_LABEL}
                </p>
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className="flex gap-6 text-on-surface-variant font-mono text-[11px] uppercase tracking-wider">
                    <span className="flex items-center gap-2">
                      <Clock className="w-3.5 h-3.5" /> Catalog
                    </span>
                    <span className="flex items-center gap-2">
                      <BarChart className="w-3.5 h-3.5" /> Live data
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-8 rounded-sm border border-dashed border-outline-variant/30 p-12 text-center text-on-surface-variant text-sm">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-surface-high rounded-sm p-6 border border-primary/10 glass-ai flex-1">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 group cursor-pointer">
                ...
                <div>
                  <p className="text-xs font-bold text-white group-hover:text-primary transition-colors">Filter by year</p>
                  <p className="text-[10px] font-mono text-on-surface-variant mt-1">Uses catalog data (client-side)</p>
                </div>
              </div>
              <div className="flex items-center gap-4 group cursor-pointer">
                <div className="w-12 h-12 rounded-sm bg-surface-low border border-outline-variant/10 flex items-center justify-center text-on-surface-variant group-hover:text-primary transition-colors">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-mono text-on-surface-variant mt-1">Show all catalog courses</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface-low rounded-sm p-6 border border-outline-variant/5 relative overflow-hidden">
            ...
            <h4 className="font-headline text-[11px] font-bold text-on-surface-variant mb-6 uppercase tracking-[0.2em]">Catalog</h4>
            <p className="text-on-surface-variant text-xs leading-relaxed">
              Lists all Computer Science courses from <span className="font-mono text-primary">GET /course</span>. Enrollment is not modeled yet.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-6 pt-8">
        <div className="flex items-end justify-between">
          <div>
            <h3 className="font-headline text-2xl font-bold text-white tracking-tight">Catalog courses</h3>
            <p className="text-on-surface-variant text-[11px] font-mono mt-1 uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            ...
            >
              <div className="h-44 w-full overflow-hidden relative">
                ...
                <div className="absolute top-3 right-3 px-2 py-0.5 rounded-sm bg-black/60 backdrop-blur text-[9px] text-white font-mono uppercase tracking-widest border border-white/10">
                  Y{course.acadamicYear}
                </div>
              </div>
              <div className="p-5 flex-1 flex flex-col">
                <h4 className="font-headline text-base font-bold text-white mb-2 line-clamp-2 group-hover:text-primary transition-colors">
                  {course.name}
                </h4>
          ...
          {filtered.length === 0 && (
            <div className="xl:col-span-2 bg-surface-low rounded-sm p-8 border border-dashed border-outline-variant/20 flex flex-col items-center justify-center text-center opacity-40 grayscale hover:opacity-60 transition-opacity">
              ...
              <p className="text-on-surface-variant text-xs max-w-xs mt-2 leading-relaxed">Try another year filter or add courses in the admin/instructor tools.</p>
            </div>
          )}
        </div>
      </section>
